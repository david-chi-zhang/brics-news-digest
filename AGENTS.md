...
| Agent ID | Workspace | 技能 | 切换命令 |
|----------|-----------|------|---------|
| `main` | `~/openclaw/workspace` | 通用 | `openclaw agents bind --agent main --bind openim` |
| `imf-weo` | `~/openclaw/workspaces/imf-weo` | imf-weo-analyzer | `openclaw agents bind --agent imf-weo --bind openim` |
| `macro-archiver` | `~/openclaw/workspaces/macro-archiver` | ima-macro-archiver, macro-archiver | `openclaw agents bind --agent macro-archiver --bind openim` |

**查看状态：** `openclaw agents list`

**注意：** 每次新 session 开始时，检查 `memory/YYYY-MM-DD.md` 了解最近的 agent 配置变更。
